          ...
          <div class="pair-cards">
            <template v-if="mode === 'edit'">
              <ClickableRegion
                ...
                :style="pairItemStyle(index, choice)"
...
<script>
  ...
  import { choiceText } from './validate';
    ...
    setup(props, { emit }) {
      ...
      function closeOpenTarget() {
        ...
        if (target?.kind === OpenTarget.DRAFT) {
          ...
          draft.value = null;
          if (choiceText(content)) addDistractor(content);
        } else if (target?.kind === OpenTarget.DISTRACTOR) {
          const choice = state.value.distractors[target.index];
          if (choice && !choiceText(choice.content)) removeDistractor(target.index);
        }
      }
      ...
      function onRemoveDistractor(choice) {
        closeOpenTarget();
        const index = state.value.distractors.indexOf(choice);
        if (index !== -1) removeDistractor(index);
      }
      ...
      watch(
        ...
        newMode => {
          ...
          if (!choiceText(state.value.prompt)) {
            openPrompt();
      ...
      const showsEmptyChoiceError = computed(() =>
        errorCodes.value.includes(ValidationError.EMPTY_CHOICE_CONTENT),
      );

      // Blankness is re-derived from the content rather than read off the
      // error's id, because an id may repeat across choices.
      function isBlank(choice) {
        return showsEmptyChoiceError.value && !choiceText(choice.content);
      }
      ...
      function isDuplicated(choice) {
        return duplicatedTexts.value.has(choiceText(choice.content));
      }
      ...
      function pairItemStyle(index, choice) {
        const hasError =
          duplicatePairIndexes.value.has(index) || isBlank(choice) || isDuplicated(choice);
      ...
      function distractorStyle(index, choice) {
        if (isDistractorOpen(index)) return {};
        const hasError = isBlank(choice) || isDuplicated(choice);
        return { borderColor: hasError ? tokens.error : tokens.fineLine };
      }
      ...
      const pairedTexts = computed(
        () => new Set(flatten(state.value.pairs).map(c => choiceText(c.content))),
      );
      ...
      function poolChipStyle(choice) {
        const isCorrect = props.showAnswers && pairedTexts.value.has(choiceText(choice.content));
        return {
